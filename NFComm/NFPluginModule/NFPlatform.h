...
//millisecond
inline uint32_t NFGetU32TimeMS()
{
	return std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
}

//random32
inline uint32_t NFU32Random()
{
    unsigned seed = chrono::system_clock::now().time_since_epoch().count();    
    std::mt19937 gran (seed);
    uint32_t u32Random = gran();
    return u32Random;
}

//random64
inline uint64_t NFU64Random()
{
    unsigned seed = chrono::system_clock::now().time_since_epoch().count();    
    std::mt19937_64 gran (seed);
    uint64_t u64Random = gran();
    return u64Random;
}
